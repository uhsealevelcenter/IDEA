...
Load CIndRA through the advertised hierarchical package routes. The IDEA
skill loader resolves each route's required shared references and modular
skills atomically, in dependency order, without filesystem discovery or
partial reads.

For a routed CIndRA request, call:

`view_skill(source="builtin", id="cindra", route="<advertised-route-id>")`

Use the exact route IDs and descriptions in the built-in skill manifest.
Do not use terminal or filesystem tools to locate CIndRA instructions. Apply
only the route relevant to the request; use `full-report` only when the user
actually requests the complete Phase 1 workflow and product assembly.
