        ...
        try:
            for chunk in respond(self):
                if (
                    ...
                    and not chunk.get("call_id")
                ):
                    call_id = last_code_call_id()
                    if call_id:
                        chunk["call_id"] = call_id
                elif chunk.get("role") == "computer" and not chunk.get("call_id"):
                    call_id = last_code_call_id()
                    if call_id and not (
                        chunk.get("type") == "console"
                ...
                if (
                    ...
                    if self.messages[-1]["role"] != "computer":
                        ...
                        call_id = last_code_call_id()
                        if call_id:
                            empty_output["call_id"] = call_id
                        self.messages.append(empty_output)
